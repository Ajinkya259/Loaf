...
builders, with Brick-specific body dimensions/palette stripped since Deskitty
...
def material(name, hexcol, rough=0.32, metallic=0.0, emit=None, emit_str=0.0,
    ...
    # Viewport display colour. The shader nodes above only drive renders and Material
    # Preview; the viewport's default Solid shading reads this instead, so without it
    # the whole model shows up flat grey (0.8) in the GUI even though it renders fine.
    m.diffuse_color = rgb(hexcol)
    m.roughness = rough
    m.metallic = metallic
    MATS[key] = m
    return m
